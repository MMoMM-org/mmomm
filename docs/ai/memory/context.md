...
3. ✅ **Hugo non-blog migration** — shipped (2026-05-11). 5 page pairs (10 files) migrated by hand from Hugo `content/{ueber-mich,jetzt,impressum,datenschutz,videos}/index.md` + `content.en/{about,now,impressum,privacy-policy,videos}/index.md` → `src/content/pages/{de,en}/<slug>.md`. Symmetric layout, `lang`+`translationKey` set on each (`about`, `now`, `impressum`, `privacy`, `videos`). Title `| MingleMangleOfMyMind` suffix stripped (PageLayout renders title as H1; redundant body H1 removed/downgraded). Hugo `{{< youtube ID "title" >}}` shortcode → inline `<iframe>` (youtube-nocookie.com, lazy, aspect-ratio 16/9 inline style, no MDX/component). Routes split mirroring posts pattern: `src/pages/[...slug].astro` now serves DE pages + special fallbacks only; new `src/pages/en/[...slug].astro` serves EN pages. New helpers in `src/utils/i18n.ts`: `pageSlug`, `pageUrl`, `getLocalisedPages`, `findPageTranslation`. Build: 50 → 60 pages clean. URLs: `/ueber-mich/`, `/jetzt/`, `/impressum/`, `/datenschutz/`, `/videos/` (DE) + `/en/about/`, `/en/now/`, `/en/impressum/`, `/en/privacy-policy/`, `/en/videos/` (EN).

After step 3 — Vault CMS bilingual config (separate work, configure-only per ADR-004 Decision 1). Open follow-up: navigation.pages in `src/config.ts` still only contains `Posts` + `GitHub` — adding About/Now/Videos/Impressum/Datenschutz would normally require Track C per-locale strings (ADR-003), but flat array might be enough if all entries get URL-localized at render. Defer until plug into header is intentional.
